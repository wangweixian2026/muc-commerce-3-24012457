...
def load_dashboard_data(base_dir,category: Path, selected_category: str = "全部") -> dict:
    ...
    if category != "全部":
        table_df = table_df[table_df["偏好品类"] == category]
    ...
    # 生成观察语句
    insight = f"流失率最高的生命周期阶段为{max_churn_row['TenureGroup']}，流失率达到{max_churn_row['流失率']:.1%}，需要对新用户群体重点做留存运营。"
